...
//					ViewState.Add("canGetCards", false);
//					pointTotal = (int)Session["ScoreTotal"]; //Gets current score
//					pointTotal++;                           //Adds One
//					Score.InnerText = pointTotal.ToString(); //Displays Score
//					Session["ScoreTotal"] = pointTotal;     //Saves Score
				}
				else {
		...
		{                                   //checks if a string contains another string
			...
			{                                                           //does it contain the string AND is it not surrounded by another string (2150 != 15)
				...
				if (!isNumber(big.Substring(s, 1)) && !isNumber(big.Substring(e, 1)))
